...
ABLATIONS = {
    ...
    "no-semantic": {"enable_embeddings": False, "enable_reranker": False, "retrieval_mode": "fts"},
    "semantic-only": {"retrieval_mode": "semantic", "enable_reranker": False},
    "hybrid-no-rerank": {"retrieval_mode": "hybrid", "enable_reranker": False},
}
...
async def main():
    ...
    ap.add_argument("--tasks", default=str(BACKEND_ROOT / "bench" / "tasks.json"))
    ap.add_argument("--db", default=str(PROJECT_ROOT / "data" / "bench.db"))
    ap.add_argument("--max-tasks", type=int, default=None)
    ap.add_argument("--all-ablations", action="store_true",
                    help="Run every ablation back-to-back.")
    args = ap.parse_args()

    out_dir = BACKEND_ROOT / "bench" / "results"
    ...
    for label in labels:
        ...
        print(f"  saved:              {out.relative_to(BACKEND_ROOT)}")
